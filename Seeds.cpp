...
{
    // a is dot product, b1 and b2 the squared norms
    float a=0.f, b1=0.f, b2=0.f;
    for (int x=-win; x<=win; x++) {
        for (int y=-win; y<=win; y++) {
            a += (im1(i1 + x, j1 + y) - m1) * (im2(i2 + x, j2 + y) - m2);
            b1 += pow(im1(i1 + x, j1 + y) - m1, 2);
            b2 += pow(im2(i2 + x, j2 + y) - m2, 2);
        }
    }
    return a / (sqrt(b1 * b2));
}
...
{
    // Check that the function is not called with wrong arguments
    assert(i >= win && i < im.width() - win && j >= win && j < im.height() - win);
    float s=0.0f;
    int w = 2*win+1;
    for (int x=-win; x<=win; x++) {
        for (int y=-win; y<=win; y++) {
            s += static_cast<float>(im(i + x, j + y));
        }
    }
    return s;
}
...
static void find_seeds(Image<byte> im1, Image<byte> im2,
    ...
    for(int y=win; y+win<maxy; y++) {
        ...
        for(int x=win; x+win<im1.width(); x++) {
            ...
            int bestDisp = dMin - 1;
            float bestNCC = 0.f;
            int start = max(static_cast<int>(dMin), win-x); // Don't go too far
            int end = min(static_cast<int>(dMax), im2.width()-win-x); // Don't go too far
            for (int disp=start; disp<end; disp++) {
                float newNCC = ccorrel(im1, x, y, im2, x + disp, y);
                if (newNCC > bestNCC) {
                    bestNCC = newNCC;
                    bestDisp = disp;
                }
    ...
    while(! Q.empty()) {
        ...
        for(int i=0; i<4; i++) {
            ...
            if(0<=x-win && 0<=y-win &&
               ...
               ! seeds(x,y)) {
                float bestNCC = -1.f; // Min ncc
                for (int dd=-1; dd<=1; dd++) { // dd is the displacement delta
                    if (dMin <= s.d + dd && s.d + dd <= dMax && win <= x+s.d+dd && x+s.d+dd < im2.width()-win) { // Check we do not go outside the pic
                        float ncc = ccorrel(im1, x, y, im2, x+s.d+dd, y);
                        if (ncc > bestNCC) {
                            disp(x, y) = s.d + dd;
                            bestNCC = ncc;
                        }
